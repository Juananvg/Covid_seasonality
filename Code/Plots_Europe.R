...
r_ini$Period <- rep("Without Inmunity", nrow(r_ini))
r_fin$Period <- rep("With Inmunity", nrow(r_fin))
...
setwd("Paper_images")
#dev.off()
jpeg(filename = "Gam_Europe_Communities.jpeg", 
...
for (i in names(res_ini_Europe$Data)){
    res <- or_gam(data = res_ini_Europe$Data[[i]], 
        model = res_ini_Europe$Models[[i]], pred = "Temp", values = c(20,25))
...
for (i in names(res_fin_Europe$Data)){
    res <- or_gam(data = res_fin_Europe$Data[[i]], 
        model = res_fin_Europe$Models[[i]], pred = "Temp", values = c(20,25))
    OR[i,1] <- res$oddsratio
    if(res$`CI_low (2.5%)` < res$`CI_high (97.5%)`){
        OR[i,2] <- res$`CI_low (2.5%)`
        OR[i,3] <- res$`CI_high (97.5%)`}
...
setwd("Paper_images")
dev.off()
jpeg(filename = "Odds_Europe.jpeg", height = 1500, width = 1500)
graf_both
dev.off()
rm(list = ls())
setwd("..")
